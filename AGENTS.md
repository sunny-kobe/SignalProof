...
- 它现在也是一个克制版 `AI Native Work Loop Protocol`：把高不确定性的个人工作压成证据、Spec、AI 执行、反馈、决策和资产；但仍不是通用 workflow builder。
...
`case_type` 表示证据边界；`case_intent` 表示工作意图。`case_intent` 可为：
...
新 case 的 `signal.md` 必须维护：

- `loop_profile: ai-work-loop-v1`
- `agentic_loop: required|optional|skipped`
- `developer_feedback_loop: required|optional|skipped`
- `external_feedback_loop: required|delayed|skipped-with-reason`
- `asset_loop: required|optional|skipped`

当 `agentic_loop=required` 时，`validation.md` 必须包含 `Product Spec` 和 `Eval Set`。这些字段只用于控制本轮 loop，不得替代研究证据或真实反馈。
